...
def run_evaluation():
    print("Initializing Re-ranking Pipeline (Python Hybrid + BGE Cross-Encoder)...")
    ...
    try:
        pipeline = RerankPipeline(qdrant_path=qdrant_path, bm25_pkl_path=bm25_path)
    ...
    print(f"\nEvaluating Re-ranking Pipeline across {total_q} queries...")
    ...
    print("\n--- Cross-Encoder Re-ranking Pipeline Evaluation Results ---")
    for k, v in results.items(): print(f"{k}: {v}")
        
    out_file = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'rerank_eval_results.json'))
